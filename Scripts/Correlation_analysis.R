...
## Cohort info ##
id<-as.data.frame(read_excel("InVitro_DrugResponseDatabase.xlsx", "AUC",check.names =F))
...
## Drug efficacy data ##
results_AUC <- read.xlsx("InVitro_DrugResponseDatabase.xlsx","AUC",check.names =F)[3:128,]
...
`%notin%` <- Negate(`%in%`)
AUC<-AUC[,colnames(AUC)  %notin% c("Trametinib","Cobimetinib","Selumetinib")] # remove biased drugs 
